-- BookNest availability fallback
-- If a business has no availability row for a selected day, treat the day as
-- open from 09:00 to 18:00 unless the date is explicitly blocked. If the owner
-- adds an availability row for that day and marks it closed, that setting wins.
