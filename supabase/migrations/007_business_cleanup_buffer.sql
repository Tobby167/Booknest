-- Adds a business-wide hidden cleanup/break buffer after appointments.
-- Clients still see only the real service duration. BookNest uses this buffer
-- when generating slots and when enforcing double-booking prevention.

alter table public.businesses
  add column if not exists default_buffer_after_minutes integer not null default 0;

alter table public.businesses
  drop constraint if exists businesses_default_buffer_after_minutes_range,
  add constraint businesses_default_buffer_after_minutes_range
    check (default_buffer_after_minutes between 0 and 720);
...
as $$
  select
    ...
    ((a.appointment_date + a.end_time)
      + make_interval(mins => coalesce(s.buffer_after_minutes, 0) + coalesce(b.default_buffer_after_minutes, 0)))::time as end_time
