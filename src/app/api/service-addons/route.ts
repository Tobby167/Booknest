...
export async function POST(request: Request) {
  ...
  const parsed = serviceAddonSchema.safeParse(await request.json());
  ...
  const { data, error } = await supabase.from("service_addons").insert(parsed.data).select("*").single();
  if (error) return fail(error.message, 500);
  return ok({ addon: data }, { status: 201 });
}
