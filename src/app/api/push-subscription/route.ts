...
// GET ΓÇö return the VAPID public key so the browser can subscribe
export async function GET() {
  ...
  console.log("[Push] GET /api/push-subscription ΓÇö returning public key (starts with):", key.slice(0, 8));
  return NextResponse.json({ publicKey: key });
}

// POST ΓÇö save a push subscription for the current logged-in user
export async function POST(req: NextRequest) {
  try {
    console.log("[Push] POST /api/push-subscription ΓÇö saving subscription");
...
// DELETE ΓÇö remove a push subscription
